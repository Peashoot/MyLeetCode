...
struct ListNode *getIntersectionNode(struct ListNode *headA, struct ListNode *headB)
{
    ...
    struct ListNode *headACount = headA, *headBCount = headB;
    int countA = 0;
    int countB = 0;
    while (headACount->next != NULL)
    {
        headACount = headACount->next;
        countA++;
    }
    while (headBCount->next != NULL)
    {
        headBCount = headBCount->next;
        countB++;
    }
